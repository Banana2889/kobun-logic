...
if st.session_state.app_mode == 'pre_mapping':
    ...
    with st.form("post_form"):
        ...
        if st.form_submit_button("保存して終了"):
            ...
            try:
                ...
                pre_dict.update({"日時": now, "タイプ": "事前", **st.session_state.pre_text, **st.session_state.pre_likert})
                
                # 事後データの行作成
                post_dict = {c: "-" for c in cols}
                post_dict.update({"日時": now, "タイプ": "事後", **st.session_state.post_text, **st.session_state.post_likert, "ログ": str(st.session_state.results)})
    ...
    st.balloons(); st.header("🎊 義経伝、読了"); st.success("データは保存されました。感謝いたします。")
